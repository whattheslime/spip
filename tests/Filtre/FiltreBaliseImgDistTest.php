...
class FiltreBaliseImgDistTest extends TestCase
{
    ...
    public function testFiltresFiltreBaliseImgDist($expected, ...$args): void
    {
        ...
        if (is_null($f) && !function_exists($f = 'filtre_balise_img_dist')) {
            find_in_path("inc/filtres.php", '', true);
            $f = chercher_filtre($f);
        }
        $res = $f(...$args);
