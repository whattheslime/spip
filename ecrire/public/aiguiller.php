...
function traiter_formulaires_dynamiques($get = false) {
	...
	if (!empty($GLOBALS['visiteur_session']['id_auteur'])) {
		if (empty($sign)) {
			spip_logger('formulaires')->error(
				"signature ajax form incorrecte : $form (formulaire non signe mais on a une session)"
			);
			return false;
		}
		...
		if ($sign !== $secu['hash']) {
			spip_logger('formulaires')->error(
				"signature ajax form incorrecte : $form (formulaire signe mais ne correspond pas a la session)"
			);
			return false;
		}
	} else {
	...
	if (isset($post["erreurs_$form"]) && (is_countable($post["erreurs_$form"]) ? count(
		$post["erreurs_$form"]
	) : 0) && !isset($post["erreurs_$form"]['message_erreur'])) {
	...
	if (isset($post["erreurs_$form"]) && (is_countable($post["erreurs_$form"]) ? count(
		...
		$rev = pipeline('formulaire_traiter', [
			'args' => ['form' => $form, 'args' => $args],
			'data' => $rev,
		]);
