...
$GLOBALS[$GLOBALS['idx_lang']] = array(
	...
	'avis_chemin_invalide_1' => 'Выбранный вами путь', # MODIF
	...
	'avis_connexion_echec_3' => '<b>N.B.</b> На многих серверах вы должны сделать запрос для включения вашего доступа к базе данных SQL перед тем, как использовать её. Если вы не можете установить соединение, убедитесь, что такой запрос действительно был сделан.', # MODIF
	...
	'avis_connexion_ldap_echec_2' => 'Вернитесь на предыдущую страницу и проверьте внесенную вами информацию.', # MODIF
	'avis_connexion_ldap_echec_3' => 'Кроме того, не используйте поддержку LDAP для импорта пользователей.',
	'avis_deplacement_rubrique' => 'Внимание! В этом разделе находится  @contient_breves@ новостей@scb@: если вы перемещаете его, пожалуйста, поставьте отметку для подтверждения операции.', # MODIF
	...
	'avis_lecture_noms_bases_2' => 'Ни одна из баз данных не доступна или функция, позволяющая внесение в список баз данных, была отключена в целях безопасности (лучше иметь большее количество хостов).', # MODIF
	...
	'avis_non_acces_page' => 'У вас нет доступа к этой странице.', # MODIF
	...
	'calendrier_synchro' => 'Если вы используете календарь, совместимый с <b> iCal </b>, вы можете согласовать его с данными сайта.', # MODIF
	...
	'config_info_base_sup' => 'Если вам необходим запрос других баз данных используя SPIP, независимо от того, находятся ли они на том же SQL сервере или где-нибудь еще, используйте форму, показанную ниже, для их описания. Если вы оставите некоторые поля пустыми, то связанные детали будут использоваться с основной базой данных.', # MODIF
	...
	'config_info_redirection' => 'Виртуальная статья - это возможность перенаправлять посетителей по произвольному URL, как другую страницу этого сайта, так и на любую ссылку в интернете.', # MODIF
	...
	'entree_biographie' => 'Об авторе', # MODIF
	...
	'entree_infos_perso' => 'Об авторе', # MODIF
	...
	'entree_login_connexion_2' => '(Иногда соответствует вашему логину  FTP доступа и иногда остается пустым)', # MODIF
	'entree_mot_passe' => 'Ваш пароль',
	'entree_mot_passe_1' => 'Пароль',
	'entree_mot_passe_2' => '(Иногда соответствует вашему паролю FTP  доступа и иногда остается пустым)', # MODIF
	...
	'entree_nom_site' => 'Название вашего сайта', # MODIF
...
Также вы можете задать константы @constantes@ (в файле mes_options.php) так, что бы эти файлы находились все папки @document_root@.', # MODIF
	...
	'ical_info_calendrier' => 'В вашем распоряжении есть два календаря. Первый - это карта сайта, показывающая все опубликованные статьи. Второй содержит редакторские объявления, а также ваши личные сообщения: он сохранен для вас, благодаря ключу, который вы можете изменить в любое время, подтвердив свой пароль.', # MODIF
	...
	'ical_texte_prive' => 'Этот календарь является строго личным, он информирует вас о работе со статьями на этом сайте (задачи, персональные настройки, отправленные статьи и новости дня ...).', # MODIF
	...
	'info_activer_cookie' => 'Вы можете включить <b>администраторский cookie</b>, который позволяет вам легко переключаться между основной и административной частью сайта.', # MODIF
	...
	'info_administrateur_2' => 'на этом сайте (<i>использовать с предостережением</i>)', # MODIF
	...
	'info_ajout_image' => 'Когда вы добавляете изображения как прикрепленные документы к  статье, SPIP может автоматически создать уменьшенную копию   вставленных изображений. Это позволит, например, автоматически создавать галерею или портфолио.', # MODIF
	...
	'info_base_installee' => 'Структура вашей базы данных установлена.', # MODIF
	'info_bio' => 'Биография', # MODIF
	...
	'info_chemin_acces_2' => 'Теперь вы должны настроить путь доступа к каталогу информации. Эта информация необходима для просмотра профилей пользователей, хранящихся в каталоге.', # MODIF
	...
	'info_config_suivi' => 'Если этот адрес соответствует списку e-mail рассылки, вы можете указать ниже адрес, по которому участники сайта могут зарегистрироваться. Этим адресом может быть адрес сайта (например страница списка регистрации через сеть) или адрес электронной почты с определенной темой (например: <tt>@adresse_suivi@?subject=subscribe</tt>):', # MODIF
	'info_config_suivi_explication' => 'Вы можете подписаться на почтовую рассылку этого сайта. Тогда вы будете получать на email информацию о статьях и новостях, отправленных для публикации.', # MODIF
	'info_confirmer_passe' => 'Подтвердите новый пароль:',
	'info_conflit_edition_avis_non_sauvegarde' => 'Внимание: следующие поля были изменены в другом месте. Поэтому ваши изменения в этих полях не были сохранены.', # MODIF
	...
	'info_connexion_ldap_ok' => '<b>Cоединение c LDAP установлено. </b> <p>Вы можете перейти к следующему шагу.</p>', # MODIF
	...
	'info_creation_rubrique' => 'Создайте хотя бы один раздел для того, чтобы создавать статьи.<br />', # MODIF
	...
	'info_derniere_etape' => 'Следующий этап: Готово!', # MODIF
	...
	'info_ecrire_article' => 'Перед тем как написать статью, вы должны создать хотя бы один раздел.', # MODIF
...
Переводы связанные с оригиналом, 
 в цвете указывающем на их статус:в цвете, указывающем их статус:',
	...
	'label_bando_outils_masquer' => 'Скрыть инструменты', # MODIF
	...
	'ldap_correspondance_2' => 'Укажите соответствующее LDAP поле для каждого из данных полей. Оставьте их пустыми, если вы не хотите, чтобы они заполнялись. Несколько LDAP полей разделяются пробелом или запятой.', # MODIF
	...
	'module_texte_explicatif' => 'Вы можете вставить следующие ярлыки в шаблон вашего сайта. Они будут автоматически переведены на те языки, для которых существует языковой файл.', # MODIF
	'module_texte_traduction' => 'Языковой файл «@module@» является доступным в:', # MODIF
	...
	'plugin_info_automatique1' => 'Для того, чтобы разрешить  автоматическую установку плагинов:', # MODIF
	...
	'plugin_info_automatique_lib' => 'Некоторые плагины должны иметь возможность загружать файлы в каталог <code>lib/</code>. Возможно, его необходимо создать самостоятельно.', # MODIF
	...
	'plugin_info_automatique_select' => 'Выберите плагин, SPIP скачает и установит его в каталог <code>@rep@</code>. Если плагин уже установлен, он будет обновлен.', # MODIF
	...
	'plugin_necessite_plugin' => 'Для этого плагина необходим @plugin@  @version@ или новее.', # MODIF
	...
	'plugin_zip_content' => 'Он содержит следующие файлы (@taille@), <br />готовые к установке в каталоге <code>@rep@</code>', # MODIF
	...
	'plugin_zip_installer' => 'Теперь вы можете установить.', # MODIF
	...
	'taille_cache_infinie' => 'На вашем сайте нет ограничений на размер кеша.', # MODIF
	'taille_cache_maxi' => 'Заданный максимальный размер файлов кеша составляет приблизительно <b> @octets@ </b>.', # MODIF
	'taille_cache_moins_de' => 'Размер кеша меньше  @octets@.',
	'taille_cache_octets' => 'Приблизительный размер кеша @octets@.',
	'taille_cache_vide' => 'Кеш пуст.', # MODIF
	'taille_repertoire_cache' => 'Текущий размер кеша', # MODIF
	'text_article_propose_publication' => 'Статья отправлена на утверждение администратору.',
	'texte_acces_ldap_anonyme_1' => 'Некоторые серверы LDAP не позволяют анонимного доступа. В этом случае вы должны указать исходный идентификатор доступа, чтобы впоследствии иметь возможность искать информацию в каталоге. Однако, в большинстве случаев следующие поля можно оставить пустыми.', # MODIF
...
переустановить SPIP, чтобы восстановить новую базу данных и доступ первого администратора.', # MODIF
	'texte_adresse_annuaire_1' => '(Если ваш каталог установлен на том же компьютере, что ваш вебсайт, это вероятно «localhost».)', # MODIF
	'texte_ajout_auteur' => 'Следующий автор был добавлен к статье:',
	'texte_annuaire_ldap_1' => 'Если вы имеете доступ к каталогу (LDAP), вы можете использовать его, для автоматического импорта пользователей под SPIP.', # MODIF
	'texte_article_statut' => 'Статус статьи:',
	'texte_article_virtuel' => 'Виртуальная статья',
	'texte_article_virtuel_reference' => '<b>Виртуальная статья:</b> статья, на которую ссылаются на вашем SPIP сайте, но переадресованная на другой адрес. Чтобы удалить перенаправление, удалите вышеупомянутый адрес.', # MODIF
	'texte_aucun_resultat_auteur' => 'Нет результатов для "@cherche_auteur@".',
	'texte_auteur_messagerie' => 'Этот сайт может постоянно контролировать список редакторов,находящихся он-лайн, который позволяет вам обмениваться сообщениями в реальном времени. Вы можете решить не появляться в этом списке (тогда вы будете "невидимы" для других пользователей).', # MODIF
	'texte_auteurs' => 'АВТОРЫ', # MODIF
	'texte_choix_base_1' => 'Выбрать Вашу базу данных:',
	'texte_choix_base_2' => 'SQL сервер содержит несколько баз данных.',
	'texte_choix_base_3' => '<b>Выберите</b> ниже ту, которую ваш интернет провайдер может отнести к вам:', # MODIF
	...
	'texte_compatibilite_html_attention' => 'При включении поддержки <i>HTML5</i> проблемы не ожидаются, но в этом случае необходимо добавить следующую строку кода на все страницы сайта: <code><!DOCTYPE html></code>.', # MODIF
	'texte_compte_element' => '@count@ элемент',
	'texte_compte_elements' => '@count@ элементы',
	'texte_conflit_edition_correction' => 'Пожалуйста, проверьте ниже разницу между двумя версиями. Вы также можете скопировать ваши изменения и начать всё заново.', # MODIF
	...
	'texte_contenu_articles' => 'Выбрать дополнительные информационные поля для статей. В зависимости от того, как запрограммирован ваш сайт, они могут отображаться или не отображаться в общем доступе. ', # MODIF
	'texte_crash_base' => 'Если ваша база данных повреждена, вы можете попробовать восстановить ее  автоматически.', # MODIF
	'texte_creer_rubrique' => 'Создайте хотя бы один раздел, чтобы писать статьи.<br />', # MODIF
...
<p>Если у вас нет необходимости в таких действиях, то оставьте это значение по умолчанию.</p>',
	...
	'texte_informations_personnelles_2' => '(Примечание: если вы переустанавливаете SPIP и ваш старый логин и пароль всё еще работают, вы можете', # MODIF
	'texte_introductif_article' => '(Вступительный текст к статье)',
	'texte_jeu_caractere' => 'Рекомендуется использовать кодировку (<tt>utf-8</tt>) на вашем сайте. Это сделает возможным отображать текст на любом языке. ', # MODIF
	'texte_jeu_caractere_3' => 'Текущая кодировка:',
	'texte_jeu_caractere_4' => 'Если это не соответствует ситуации, которую вы имеете с данными (например, после восстановления базы данных с резервной копии), или если <em>вы создаете этот сайт</em> и хотите использовать разные наборы символов, пожалуйста, укажите набор символов здесь:', # MODIF
 ...
 Теперь вы должны обновить базу данных сайта.', # MODIF
	...
	'texte_nouvelle_version_spip_2' => 'Чтобы обновить сайт до новой версии, то, если вы - вебмастер этого сайта, пожалуйста, удалите файл @connect@ и повторно начните установку, чтобы обновить настройки подключения к базе данных.<p>(NB.: если вы забыли свои параметры соединения, то они хранятся в файле @connect@ </p>', # MODIF
	'texte_operation_echec' => 'Вернитесь к предыдущей странице, выберите другую базу данных или создайте новую. Проверьте информацию, предоставленную вашим хостом.', # MODIF
	...
	'texte_proposer_publication' => 'Когда ваша статья закончена, <br />вы можете представить ее для публикации.', # MODIF
  ...
  <tt> <html> http: // proxy:8080 </HTML> </tt>. В большинстве случаев Вы можете оставить это поле пустым.',
	...
	'texte_recalcul_page' => 'Если вы хотите обновить только одну страницу, вы можете сделать это в основной части, используя кнопку, "обновить".', # MODIF
	'texte_recuperer_base' => 'Востановить базу данных',
	'texte_reference_mais_redirige' => 'ссылка на статью вашего SPIP сайта, но переадресована на другой адрес.', # MODIF
...
<li> язык ,по умолчанию, для   административной части. </li> </ul>',
	...
	'texte_suppression_fichiers' => 'Используйте команду для удаления всех файлов из кеша SPIP.  Это позволит обновить все страницы сайта. Это очень удобно, если вы изменили шаблоны или обновили значительное количество информации на сайте.', # MODIF
	'texte_sur_titre' => 'Главное название',
	'texte_table_ok' => ': эта таблица готова.', # MODIF
	'texte_tentative_recuperation' => 'Попытка восстановления ',
	'texte_tenter_reparation' => 'Попытка восстановления базы данных',
	'texte_test_proxy' => 'Чтобы попробовать прокси, введите здесь адрес вебсайта, который вы хотите проверить.', # MODIF
	'texte_titre_02' => 'Заглавие:', # MODIF
	'texte_titre_obligatoire' => '<b>Заглавие</b> [обязательно]', # MODIF
	'texte_travail_article' => '@nom_auteur_modif@ работал над этой статьей @date_diff@ минут назад',
	'texte_travail_collaboratif' => 'Если несколько авторов редактируют одну и ту же статью, то система отмечает ее как "в работе" и ограничивает доступ к ней, чтобы избежать потери информации вследствие одновременного редактирования материала. По умолчанию эта возможность отключена.', # MODIF
	'texte_vide' => 'очистить',
	'texte_vider_cache' => 'Очистить кеш', # MODIF
